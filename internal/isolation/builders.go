...
//
// The profile's level is enforced as declared — including "restricted". The
// tenant's k3s control plane runs in its own dedicated namespace (under
// "baseline", since k3s runs as root like upstream), so the workload namespace
// no longer needs to accommodate it. A profile that says "restricted" means
// restricted: tenant pods that do not satisfy it are rejected by admission.
